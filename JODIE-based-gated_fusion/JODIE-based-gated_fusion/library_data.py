...
def load_network(args, time_scaling=True):
    ...
    # Get timediff scale method from args
    ...
    if time_scaling:
        print(f"Scaling timestamps using method: {timediff_scale_method}")
        
        if timediff_scale_method == 'standard':
            # Standard scaling: scale(timediff + 1)
        ...
        elif timediff_scale_method == 'log':
            # Log scaling: log1p(timediff)
            user_timedifference_sequence = np.log1p(np.array(user_timedifference_sequence))
            item_timedifference_sequence = np.log1p(np.array(item_timedifference_sequence))
            
        elif timediff_scale_method == 'log_minmax':
            # Log scaling then 0-1 normalization (separate max_val for user and item)
            user_timedifference_sequence = np.log1p(np.array(user_timedifference_sequence))
            item_timedifference_sequence = np.log1p(np.array(item_timedifference_sequence))
            
            # Separate normalization if --timediff_separate_encoder is set
            separate_encoder = getattr(args, 'timediff_separate_encoder', False)
            if separate_encoder:
                user_max_val = np.max(user_timedifference_sequence)
                item_max_val = np.max(item_timedifference_sequence)
                user_timedifference_sequence = user_timedifference_sequence / user_max_val
                item_timedifference_sequence = item_timedifference_sequence / item_max_val
            else:
                # Shared normalization
                max_val = np.max([np.max(user_timedifference_sequence), np.max(item_timedifference_sequence)])
                user_timedifference_sequence = user_timedifference_sequence / max_val
                item_timedifference_sequence = item_timedifference_sequence / max_val
